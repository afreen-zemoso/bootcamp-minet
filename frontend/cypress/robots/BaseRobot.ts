...
export class BaseHands {
    ...
    typeTextonId(id: string, text: string) {
        cy.get(`#${id}`).clear().type(text, { force: true });
        return this;
    }
...
export class BaseDependencies {
    ...
    login() {
        ...
        cy.visit(`${url}`);
        cy.get('#email').type(constants.VALIDEMAIL);
        cy.get('#password').type(constants.VALIPASSWORD);
